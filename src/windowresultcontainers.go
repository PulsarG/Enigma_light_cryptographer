...
func (r *Resulter) createContainerWithButtons() *fyne.Container {
	saveFileButton := elem.NewButton(consts.BUTTON_RESULTWINDOW_SAVE_FILE, r.saveFile)

	txtBound := binding.NewString()
	txtBound.Set(r.code)

	copyFileButton := widget.NewButtonWithIcon(consts.BUTTON_RESULTWINDOW_COPY, theme.ContentCopyIcon(), func() {
